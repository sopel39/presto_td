...
public class TestHashJoinOperator
{
            ...
            throws Exception
    {
        ...
        DriverContext ljoDriverContext = taskContext.addPipelineContext(2, true, true).addDriverContext();
        ...
        InternalJoinFilterFunction filterFunction = new TestInternalJoinFilterFunction(
                (leftPosition, leftBlocks, rightPosition, rightBlocks) -> {
                    called.set(true);
                    ljoDriverContext.getYieldSignal().forceYieldForTesting();
                    return true;
                });
        ...
        try (OperatorFactory joinOperatorFactory = innerJoinOperatorFactory(lookupSourceFactory, probePages, joinSpillerFactory);
                Operator joinOperator = joinOperatorFactory.createOperator(ljoDriverContext)) {
            // build lookup source
            ...
            while (!called.get()) { // process first row of first page of LookupJoinOperator
                processRow(ljoDriver, taskStateMachine);
            }
    ...
    {
        SPILL_HBO, SPILL_LJO, UNSPILL_HBO, UNSPILL_LJO
    }
    ...
    private static void triggerMemoryRevokingAndWait(Operator operator, TaskStateMachine taskStateMachine)
            throws Exception
    {
